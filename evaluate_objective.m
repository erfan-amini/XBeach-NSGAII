function f = evaluate_objective(x,M, wave_power,Dataset,lb,ub)
...
Robustness = FFF_GEP_Robustness(wave_power,x);
[~,~,~,serviceability] = FFF_outputs_box_averaging(wave_power,x,Dataset);
f(1) = -Robustness;
f(2) = -serviceability;
f(3) = FFF_Cost(x,lb,ub);
